...
namespace Tesseract.Core.Logic.Implementation
{
    ...
    public class DefaultInputValidationLogic : IInputValidationLogic
    {
        ...
        {
            if (tag.IsNullOrWhitespace()) return new ApiValidationError(nameof(tag), ErrorKeys.ArgumentCanNotBeEmpty);
        ...
        {
            if (ns.IsNullOrWhitespace()) return new ApiValidationError(nameof(ns), ErrorKeys.ArgumentCanNotBeEmpty);
        ...
        {
            if (weight < 0d) return new ApiValidationError(nameof(weight), ErrorKeys.ArgumentCanNotBeNegative);

            if (double.IsNaN(weight)) return new ApiValidationError(nameof(weight), ErrorKeys.ArgumentIsNotANumber);
        ...
        {
            if (input < min) return new ApiValidationError(fieldName, ErrorKeys.ArgumentIsSmallerThanMinimum);

            if (input > max) return new ApiValidationError(fieldName, ErrorKeys.ArgumentIsLargerThanMaximum);
        ...
        public ApiValidationResult ValidateAccountQuery(AccountQuery query)
        {
            ...
            if (numberOfConditions > 512) result.Append(new ApiValidationError(ErrorKeys.QueryHasTooManyConditions));
        ...
        {
            if (query == null) return ApiValidationResult.Failure(nameof(query), ErrorKeys.ArgumentCanNotBeEmpty);

            if (depth >= 8) return ApiValidationResult.Failure(ErrorKeys.QueryNestingIsTooDeep);
            ...
            if (query.Not != null) result.Append(ValidateAccountQuery(query.Not, depth + 1, ref numberOfConditions));
